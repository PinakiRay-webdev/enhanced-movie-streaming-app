...
const TvShowDes = () => {
    ...
    const getMovieDetail = async () =>{
        try {
            const response = await axios.get(`${baseURL}/tv/${showID}?api_key=${api_key}`)
    ...
    const getTitleImage = async () =>{
        try {
            const response = await axios.get(`${baseURL}/tv/${showID}/images?api_key=${api_key}`)
                ...
                <div className='show-detail-banner absolute w-full h-full top-0 flex items-center pl-[14rem]' >
                    <div className='flex gap-8 items-end' >
                        ...
                        <header className='flex items-center mb-4 justify-between' >
                                <p className='text-white font-bold opacity-60 text-xl' >{`${currentShow.first_air_date}`.slice(0,4)}</p>
                        ...
                        <div>
                            ...
                            <footer className='mt-12 flex items-end justify-between'>
                                ...
                                <div className='flex items-end gap-4' >
                                    <p className='text-[#ace4ec] text-4xl' >{currentShow.number_of_seasons}</p>
                                    <p className='text-white font-sans font-extralight' >No. of seasons</p>
                                </div>
                            </footer>
                        </div>
                    </div>
