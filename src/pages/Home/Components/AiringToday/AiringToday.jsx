...
const AiringToday = () => {
  ...
  const [airedToday, setAiredToday] = useState([]);

  const getTodaysAired = async () => {
    try {
      const response = await axios.get(
        `${baseUrl}/tv/airing_today?api_key=${apiKey}`
      );
      setAiredToday(response.data.results);
    } catch (error) {
  ...
  return (
    <div className="px-10 py-3">
      <div className="flex justify-between" >
        <h1 className="text-amber-400 text-4xl">Today's Airing</h1>
      ...
      <div ref={scrollBox} className="latestMovies my-3 flex gap-4 overflow-x-scroll scroll-smooth">
        {airedToday?.map((Element, id) => (
          <div onClick={() =>navigate(`/show/${Element.id}`)} className="cursor-pointer" key={id}>
            <img
